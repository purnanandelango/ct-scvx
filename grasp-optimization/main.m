...
prb = problem_data_cvx(20,10);


yalmip clear

x_scl = sdpvar(prb.nx,prb.K);
u_scl = sdpvar(prb.nu,prb.K-1);

x = prb.Sx*x_scl + repmat(prb.cx,[1,prb.K]);
u = prb.Su*u_scl + repmat(prb.cu,[1,prb.K-1]);

cnstr = [];
cost_fun = 0;
for k = 1:prb.K-1
    cost_fun = cost_fun + norm(u(prb.idx{1},k)) ...
                        + norm(u(prb.idx{2},k)) ...
                        + norm(u(prb.idx{3},k)) ...
                        + norm(u(prb.idx{4},k));
    cnstr = [cnstr;
