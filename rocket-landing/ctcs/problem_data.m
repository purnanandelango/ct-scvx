function prb = problem_data(K,scp_iters,wvc,wtr,cost_factor)
    ...
    prb.omgmax  = 0.3752; % 21.5*pi/180;                                              % Angular velocity (inf-norm 28.6)
    ...
    % 5: angular rate
    % 6: thrust pointing
    % 7: thrust upper bound
    % 8: thrust lower bound
    ...
    prb.cnstr_buffer = cnstr_selector * [...
                        0;
                        0;
                        0;
                        0;
    ...
    prb.cnstr_fun       = @(xi,TB) cnstr_selector * prb.cnstr_scl*[prb.mdry - xi(1);
    ...
    prb.cnstr_fun_jac_xi = @(xi,TB) cnstr_selector * prb.cnstr_scl*[-1, zeros(1,13);
    ...
    prb.cnstr_fun_jac_TB = @(xi,TB) cnstr_selector * prb.cnstr_scl*[ zeros(1,3);
                                                    zeros(1,3);
                                                    zeros(1,3);
                                                    zeros(1,3);
                                                    zeros(1,3);
...
%     prb.solver_settings = sdpsettings('solver','gurobi','verbose',false,'gurobi.OptimalityTol',1e-9,'gurobi.FeasibilityTol',1e-9);
    ...
    prb.solver = struct('name',"pipg",'eps_abs',1e-9,'verbose',0,'max_iter',5e4,'rho',1.5,'lambda',1,'omega',100.0,'test_termination',500);
